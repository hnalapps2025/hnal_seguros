...
class Pacientes{
						...
						if ($conn->connect_errno) {
    							...
    							exit();
        						    ...
        						    if($tipo!="" &&  $desde!="" && $hasta!=""){
										...
										*/
										$sql = "SELECT `procedimiento`,(SELECT NombreCorto FROM tbl_tipoEstudioProced WHERE idtpo=T.procedimiento) AS PROC,COUNT(procedimiento) AS CANTI
        					    			  FROM `tbl_registroPacientesPatologia` AS T  GROUP BY PROC ORDER BY CANTI DESC";
										...
										if($tipo!="" &&  $desde!="" && $hasta!=""){
											if($tipo == '2'){
												...
												$sql="SELECT 
														datos.id AS procedimiento,       
														PROC.NombreCorto AS PROC,        
														SUM(CANTI) AS CANTI        
													FROM (
														SELECT procedimiento AS id, COUNT(*) AS CANTI
														FROM tbl_registroPacientesPatologia
														WHERE tipoEstudio = '2' AND  `tipoEstudio`= '$tipo' AND `fechaRecepcion` BETWEEN '$desde' AND '$hasta' 
														GROUP BY procedimiento

														UNION ALL

														SELECT subproc AS id, COUNT(*) AS CANTI
														FROM tbl_registroPacientesPatologia
														WHERE tipoEstudio = '2' and  `tipoEstudio`= '$tipo' AND `fechaRecepcion` BETWEEN '$desde' AND '$hasta' 
														GROUP BY subproc
													) AS datos
													LEFT JOIN tbl_tipoEstudioProced AS PROC
														ON PROC.idtpo = datos.id
													WHERE PROC.NombreCorto IS NOT NULL
													GROUP BY datos.id, PROC.NombreCorto
													ORDER BY CANTI DESC;
														";
											}else{
												$sql="SELECT `procedimiento`,(SELECT NombreCorto FROM tbl_tipoEstudioProced WHERE idtpo=T.procedimiento) AS PROC,COUNT(procedimiento) AS CANTI
        					     						 FROM `tbl_registroPacientesPatologia` AS T WHERE `tipoEstudio`= '1' AND `tipoEstudio`= '$tipo' AND `fechaRecepcion` BETWEEN '$desde' AND '$hasta'  GROUP BY PROC ORDER BY CANTI DESC";
											}
											...
											if($tipo == '2'){
												...
												$sql="SELECT `procedimiento`,(SELECT NombreCorto FROM tbl_tipoEstudioProced WHERE idtpo=T.procedimiento) AS PROC,COUNT(procedimiento) AS CANTI
        					     						 FROM `tbl_registroPacientesPatologia` AS T WHERE `tipoEstudio`= '1'  GROUP BY PROC ORDER BY CANTI DESC";
											}
											...
											$where = "WHERE `fechaRecepcion` BETWEEN '$desde' AND '$hasta' ";
											$sql="SELECT `procedimiento`,(SELECT NombreCorto FROM tbl_tipoEstudioProced WHERE idtpo=T.procedimiento) AS PROC,COUNT(procedimiento) AS CANTI
        					     						 FROM `tbl_registroPacientesPatologia` AS T WHERE  `fechaRecepcion` BETWEEN '$desde' AND '$hasta'    GROUP BY PROC ORDER BY CANTI DESC";
										}
									//var_dump($where);
									//exit();
									...
									if ($tipo != "" && $desde != "" && $hasta != "") {
										...
										$where = "WHERE T.tipoEstudio = '$tipo' AND T.fechaRecepcion BETWEEN '$desde' AND '$hasta'";
									} else if ($tipo != "") {
										if ($tipo == '2') {
											$usarSubproc = true;
										}
									$where = "WHERE T.tipoEstudio = '$tipo'";
									} else if ($desde != "" && $hasta != "") {
										$where = "WHERE T.fechaRecepcion BETWEEN '$desde' AND '$hasta'";
									}
										*/
                        ...
                        function countLisProcedimientos($tipo,$uno,$desde,$hasta){
        						...
        						if ($conn->connect_errno) {
            							...
            							exit();
                					      ...
                					      $sql = "SELECT * FROM `tbl_registroPacientesPatologia` WHERE $where_proc= '$tipo' $where ";
										  
										  /*
										  
										  $sql = "
											SELECT * FROM tbl_registroPacientesPatologia
											WHERE procedimiento = '$tipo' $where

											UNION ALL

											SELECT pd.* FROM tbl_registroPacientesPatologia_Detalle pd
											INNER JOIN tbl_registroPacientesPatologia p ON pd.idPato = p.idPato
											WHERE pd.procedimiento = '$tipo' $where
											";
											*/
                					      $result = $conn->query($sql);
               ...
               /* function verUserPaquete($id){
					...
					function consultaXinmunohisto($id){
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								`fechaRecepcion`, `corPat`, `anaMuestraCito`, `procedRealCito`,(SELECT nom FROM usuarios WHERE id= T.medicoSolcit) AS MEDSOCI, `medicoSolcit`, `dxClinicoHi`, `interpretacPato`,(SELECT nom FROM usuarios WHERE id=T.idReportPdf) AS USRE3,
								`comentarioPatol`, `notaPatol`,(SELECT nom FROM usuarios WHERE id=T.idGeneraInforme) AS USRE,(SELECT nom FROM usuarios WHERE id=T.idAuditorAsignado) AS USRESPEC,
								idReportPdf,fechaHoraReport,fechaidGeneraInforme,fechaidReportPdf,fechaHoraCerEspe,
								(SELECT dx FROM tbl_extensionihq  WHERE idreg='$id') AS DX,
								(SELECT inter FROM tbl_extensionihq  WHERE idreg='$id') AS INP,
								(SELECT comentario FROM tbl_extensionihq  WHERE idreg='$id') AS CNM,
								(SELECT nota FROM tbl_extensionihq  WHERE idreg='$id') AS NOA,
								(select nombre from tbl_tipoEstudio tt where idTi = T.tipoEstudio) as tipoEstudioDescripcion,
							      CASE
								 WHEN T.procedimiento = 13 THEN 'CITLOGIA CERVICO-VAGINAL'
							        WHEN T.procedimiento = 19 THEN (SELECT NombreCorto FROM tbl_tipoEstudioProced WHERE idtpo = T.subproc)
							        ELSE (SELECT NombreCorto FROM tbl_tipoEstudioProced WHERE idtpo = T.procedimiento)   
					...
					function consultaReporteQuirugico($tipoEstudio,$ReporteDesde,$ReporteHasta){
						$fechaActual = date('Y-m-d');

						$hora_inicio = $ReporteDesde . ' 06:00:00';  
						$hora_fin = $ReporteHasta . ' 23:59:00';
						$db = new Conectar();
						$conn = $db->conexion();		
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								$sql = "SELECT  `tr`.`idPato`,`tr`.`corPat`,`tr`.`paciente` , `tr`.`procedimiento` , `t`.`descripcion` as servicio, `u`.`nom` as auditor, `tr`.`fechaReg` AS fecha, `tmp`.`muestra`
								FROM `tbl_registroPacientesPatologia` AS tr  left join
									`tbl_muestraPatologia` AS tmp on `tr`.`idPato` = `tmp`.`idMe` left join 
									`usuarios` as u on   `tr`.`idAuditorAsignado` = `u`.`id`  left join
									tipoServicio  as t on   `tr`.`tipoServicoPatl` = `t`.`idSer` 
								
								 WHERE `tr`.`tipoEstudio` = '1' AND  `tr`.`fechaReg` BETWEEN '$hora_inicio' AND '$hora_fin' order by  `tr`.`fechaReg` asc ;";
					...
					function consultaReporteQuirugicoMuestras($idPato){
						$fechaActual = date('Y-m-d');
						// $fechaActual  ='2025-03-17';
						$hora_inicio = $fechaActual . '06:00:00';  
					$hora_fin = $fechaActual . '19:00:00';
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								$sql = "SELECT `muestra` FROM `tbl_muestraPatologia` WHERE `idMe` = '$idPato'";
					...
					function consultaReporteCuelloUterino($tipoEstudio,$ReporteDesde,$ReporteHasta){
						$fechaActual = date('Y-m-d');
						
						$hora_inicio = $ReporteDesde . ' 06:00:00';  
						$hora_fin = $ReporteHasta . ' 23:00:00';
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								$sql = "SELECT  `tr`.`corPat`, `tr`.`paciente` , `t`.`descripcion` as servicio, `u`.`nom` as auditor, `tr`.`fechaReg` AS fecha, `tmp`.`muestra`
								FROM `tbl_registroPacientesPatologia` AS tr  left join
									`tbl_muestraPatologia` AS tmp on `tr`.`idPato` = `tmp`.`idMe` left join 
									`usuarios` as u on   `tr`.`idAuditorAsignado` = `u`.`id`  left join
									tipoServicio  as t on   `tr`.`tipoServicoPatl` = `t`.`idSer` 
						
								 WHERE `tr`.`tipoEstudio` = '2'  AND `tr`.`procedimiento` = '13' AND  `tr`.`fechaReg` BETWEEN '$hora_inicio' AND '$hora_fin' order by  `tr`.`fechaReg` asc ;";
					...
					function consultaReportePunciones($tipoEstudio,$ReporteDesde,$ReporteHasta){
						$fechaActual = date('Y-m-d');
						//$fechaActual = '2025-01-29';
						$hora_inicio = $ReporteDesde . ' 06:00:00';  
						$hora_fin = $ReporteHasta . ' 23:00:00';
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								$sql = "SELECT `tr`.`corPat`, `tr`.`paciente`  , `tr`.`procedimiento`, `tr`.`subproc`,`t`.`descripcion` as servicio, `u`.`nom` as auditor, `tr`.`fechaReg` AS fecha, `tmp`.`muestra`
								FROM `tbl_registroPacientesPatologia` AS tr  left join
									`tbl_muestraPatologia` AS tmp on `tr`.`idPato` = `tmp`.`idMe` left join 
									`usuarios` as u on   `tr`.`idAuditorAsignado` = `u`.`id`  left join
									tipoServicio  as t on   `tr`.`tipoServicoPatl` = `t`.`idSer` 
								
								 WHERE `tr`.`tipoEstudio` = '2' AND `tr`.`procedimiento` = '19' AND `tr`.`fechaReg` BETWEEN '$hora_inicio' AND '$hora_fin' order by  `tr`.`fechaReg` asc ;";
					...
					/*function consultaXRotulosPdf($forma,$tipoest){
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								$sql = "SELECT `rotulo`,`descripcion`,`descMicro` 
								FROM `tbl_observacionesRotulo` 
								WHERE `formatoPatoMac`='$forma' AND  `tipoEstudio`='$tipoest'";
					...
					function consultaXRotulosPdf($forma,$tipoest){
						...
						if ($conn->connect_errno) {
							...
							exit();
								...
								$sql = "SELECT O.`rotulo`,O.`descripcion`,O.`descMicro`,calidaMuestra,
								 SistemaReporte.Sistema AS RE,
								ParteSistemaRep.ParteSistema AS TE,
								ClasifReporte.Descripcion as  CLA,
								tbl_cptsCitologia.descripcion AS DXI
								FROM `tbl_observacionesRotulo` as O 
								LEFT JOIN SistemaReporte ON  O.hallazgo = SistemaReporte.Id
								LEFT JOIN ParteSistemaRep ON O.sisReporte = ParteSistemaRep.Id
								LEFT JOIN ClasifReporte ON O.clasificacion = ClasifReporte.Id
								LEFT JOIN  tbl_cptsCitologia ON O.idRo = tbl_cptsCitologia.code
								
								WHERE O.`formatoPatoMac`='$forma' AND  O.`tipoEstudio`='$tipoest'";
					...
					function mostrarPdfCervicoVaginal($id){
						...
						if ($conn->connect_errno) {
							...
							`obsExamenGinec`,(SELECT UPPER(`Descripcion`) FROM `TipoDxCU` WHERE IdTipoDxCU = T.`calidEspec`) AS CE,`calidEspec`, `especifiqueCalidadEspec`,`EspecCelulasEndoCervicales`,
