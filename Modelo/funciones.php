...
class Model{
				 ...
				 function InsertFilesUpload($files){
					...
					// CAMBIO CREACION PAQUETES 
						
            						function expotarCierreChange($ux,$inicio,$final){
            							...
            							if ($conn->connect_errno) {
                                         ...
                                         $fin= $final." 23:59:59";
            								...
            								/*$sql = "SELECT P.`idEm`,(SELECT `nom` FROM `usuarios` WHERE `id` =P.`idUserRegistro`) as pax , P.`nroFua`, P.`historiaClinica`,
                                        	   ...
                                        	   $sql = "SELECT CONCAT (date_format(P.`fechaIngreso`, '%d%m%Y'),date_format(P.`fechaRegistro`, '%d%m%Y')) AS FET, P.`fechaIngreso`, P.`fechaRegistro` 
                                        	   FROM `tbl_Emergencias` P WHERE  `tipoRegistro` = 3 
                                        	   AND fechaRegistro >= '$ini' AND fechaRegistro <='$fin' AND P.idEm NOT IN (SELECT idPac FROM `listadoAtencionesCE`) GROUP BY FET ORDER BY fechaRegistro DESC";
            						...
            						function expotarCierreChangeFilas($ux,$codMd5){
            							...
            							if ($conn->connect_errno) {
                                         ...
                                         $fin= $final." 23:59:59";
                                        	   ...
                                        	   (SELECT `nombre`FROM `tbl_iafas` WHERE `idIa`=P.seguro) AS IAF ,P.`cuenta`, P.`nroAfiliacion`, P.`nombres`, P.`fechaNac`, P.`ApePaterno`, 
                                        	   P.`ApeMaterno`, P.`fechaIngreso`, P.`fechaRegistro`,UPPER(status) AS ESX,(SELECT `nombre` FROM `tbl_plansalud` WHERE `idPa` =P.`planSal`) AS PS,
                                        	   regServiceCE	FROM `tbl_Emergencias` P WHERE  `tipoRegistro` = 3 AND 
                                        	   CONCAT(date_format(P.`fechaIngreso`, '%d%m%Y'),date_format(P.`fechaRegistro`, '%d%m%Y')) ='$codMd5' 
                                        	   AND P.idEm NOT IN (SELECT idPac FROM `listadoAtencionesCE`) ORDER BY fechaRegistro DESC  "; 
                                        	   
                                        	   /*
                                        	   
                                        	   SELECT P.`idEm`,(SELECT `nom` FROM `usuarios` WHERE `id` =P.`idUserRegistro`) as pax , P.`nroFua`, P.`historiaClinica`, 
                                        	   (SELECT `nombre`FROM `tbl_iafas` WHERE `idIa`=P.seguro) AS IAF ,P.`cuenta`, P.`nroAfiliacion`, P.`nombres`, P.`fechaNac`, P.`ApePaterno`, 
                                        	   P.`ApeMaterno`, P.`fechaIngreso`, P.`fechaRegistro`,UPPER(status) AS ESX,(SELECT `nombre` FROM `tbl_plansalud` WHERE `idPa` =P.`planSal`) AS PS, 
                                        	   regServiceCE FROM `tbl_Emergencias` P WHERE `tipoRegistro` = 3 AND CONCAT(date_format(P.`fechaIngreso`, '%d%m%Y'),date_format(P.`fechaRegistro`, '%d%m%Y')) 
                                        	   ='0809202313112023' AND P.idEm NOT IN (SELECT idPac FROM `listadoAtencionesCE`) ORDER BY fechaRegistro DESC;
            						...
            						function crearPaqueteCeChange($ux,$codeMd5,$fchaHoy){
            						...
            						function obtenerIdPaqueteCeChange($ux,$codeMd5){
            						...
            						function insertFuasPaqueteCeChange($idPaq,$idPac,$userReg){
